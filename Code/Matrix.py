...
#        Pr[perm_r, np.arange((self.n-1)**(self.d))] = 1
        Pr = sp.csc_matrix((np.ones((self.n-1)**self.d), (perm_r, np.arange((self.n-1)**(self.d)))))
        Pc = sp.csc_matrix((np.ones((self.n-1)**self.d), (perm_c, np.arange((self.n-1)**(self.d)))))
        b = Pr*b
        if tri == 1:
...
#    Pr = sp.csc_matrix((np.ones((n-1)**d), (perm_r, np.arange((n-1)**(d)))))
